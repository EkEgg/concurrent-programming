...
#define QTDE_OPS 10 //quantidade de operacoes sobre a lista (insercao, remocao, consulta)
...
void* tarefa(void* arg) {
   ...
   for(long int i=id; i<QTDE_OPS; i+=nthreads) {
      op = rand() % 100;
      if(op<98) {
         printf("%ld pedindo para ler.\n", id);
	 LockForRead(&rwlock); /* lock de LEITURA */    
         printf("%ld lendo.\n", id);
         Member(i%MAX_VALUE, head_p);   /* Ignore return value */
         printf("%ld deixando de ler.\n", id);
	 UnlockForRead(&rwlock);     
	 read++;
      } else if(98<=op && op<99) {
         printf("%ld pedindo pare escrever.\n", id);
	 LockForWrite(&rwlock); /* lock de ESCRITA */    
         printf("%ld escrevendo.\n", id);
         Insert(i%MAX_VALUE, &head_p);  /* Ignore return value */
         printf("%ld deixando de escrever.\n", id);
	 UnlockForWrite(&rwlock);     
	 in++;
      } else if(op>=99) {
         printf("%ld pedindo pare escrever.\n", id);
	 LockForWrite(&rwlock); /* lock de ESCRITA */     
